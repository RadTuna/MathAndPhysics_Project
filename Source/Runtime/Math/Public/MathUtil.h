...
namespace CK
{
    ...
    struct Math
    {
        ...
        FORCEINLINE static float InterpolateFloat(float current, float target, float speed, float deltaTime)
        {
            if (speed <= 0.f)
            {
                return target;
            ...
            const float deltaMove = distance * Clamp<float>(deltaTime * speed, 0.f, 1.f);
